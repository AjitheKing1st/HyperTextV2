...
function Typing() {
    ...
    const [paragraphs, setParagraphs] = useState(sentence);
    ...
    useLayoutEffect(() => {
        ...
        for (let i = 0; i < paragraphs.length; i += 1) {
            paragraphs[i].querySelector(".letter").classList.add("active")
        }
        ...
        for (let i = 0; i < paragraphs.length; i += 1) {
            inputField[i].addEventListener("input", () => {
                initTyping(paragraphs[i], inputField[i], inputField[i].value.length)
            })
        }


        function initTyping(paragraph, inputField) {

            let characters = paragraph.querySelectorAll(".letter")
            let typedChar = inputField.value.split("")[charIndex];

            console.log(charIndex, typedChar)
                        ...
                        <div className="paragraphs">
                            {paragraphs.map((paragraph, index) =>
                                <div key={index} className="paragraph">
                                    ...
                                    {paragraph.split(/(?<=\s+)/).map((letter, index) =>
